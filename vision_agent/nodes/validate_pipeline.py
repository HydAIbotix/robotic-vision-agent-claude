...
def classify_wrong_screen_failure(image_path: str, step_description: str,
    ...
    try:
        ...
        prompt = (
            ...
            "  • \"gap\" — the app looks HEALTHY and functional and simply needs more navigation to reach the "
            "expected screen. This INCLUDES the very common case where the action clearly SUCCEEDED but the "
            "UI does not auto-navigate: e.g. an item was added and a cart/basket COUNTER or badge increased "
            "(a 'Cart/Checkout (1)', 'Bag (1)' or similar control is now shown), a success toast appeared, or "
            "a 'Next'/'Continue'/'Proceed'/'View cart' control is available — the user just has to click that "
            "next control. It also covers being on an earlier/other normal screen (a list, home/login, a step "
            "along the way). No error or refusal is visible.\n\n"
            "Be conservative about \"defect\": choose it ONLY when you can SEE a refusal, error, warning/"
            "validation popup or a broken state. A plain, healthy page — ESPECIALLY one showing that the "
            "action took effect (an incremented cart count, an added item) with a clear control to continue — "
            "is a \"gap\", not a defect.\n"
            "Also rate your CONFIDENCE in the verdict as \"high\", \"medium\" or \"low\". Use \"high\" for "
            "\"gap\" when the screen is clearly healthy and offers an obvious way forward (a visible cart/"
            "next/continue control, or evidence the action succeeded), even if the expected screen is not yet "
            "shown. Use \"low\" only when it is genuinely ambiguous or you can barely tell.\n"
