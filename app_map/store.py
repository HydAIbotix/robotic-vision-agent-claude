...
def version_hash(app_map: Optional[AppMap]) -> str:
    ...
    if not app_map:
        return "no_map"
    # CONTENT-based, NOT timestamp-based. Previously this hashed `explored_at`, so EVERY re-exploration
    # bumped the version → the plan cache key changed → every cached plan was regenerated by the (then
    # non-deterministic) planner, so the same test case produced different plans across re-explores.
    # Hash the STRUCTURE instead — screen ids + each screen's sorted element ids — so a re-exploration
    # that charts the SAME screens/elements yields the SAME version and the cached plan is reused
    # verbatim. It changes only when a screen or element is actually added/removed (then a re-plan is
    # correct). Element coordinates are intentionally excluded so vision's per-pixel jitter between
    # explorations doesn't needlessly invalidate plans (tap/type focus by testid anyway).
    screens = app_map.get("screens") or {}
    parts = []
    for sid in sorted(screens.keys()):
        els = (screens.get(sid) or {}).get("elements") or []
        eids = ",".join(sorted(str(e.get("id", "")) for e in els))
        parts.append(f"{sid}:{eids}")
    import hashlib
